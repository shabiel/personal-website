ZZLEXXBD ;SLC/KER - Import - ICD-10-PCS - Lex AD/FR/BR/RU ;10/26/2017
...
LEX ; Lexicon
 ...
 D:$G(ACT)="IA" IA^ZZLEXXBE(CODE,LSTAD,LEXSRC) Q:$G(ACT)="IA"  D:$G(ACT)="RA" RA^ZZLEXXBE(CODE,LSTAD,LEXSRC) Q:$G(ACT)="RA"
 ...
 D:$G(ACT)="IA" IA^ZZLEXXBE(CODE,LSTAD,LEXSRC) D:$G(ACT)="RA" RA^ZZLEXXBE(CODE,LSTAD,LEXSRC) D:"^AD^FR^BR^RU^"[("^"_$G(ACT)_"^") AFBR
...
AFBR ; AD/FR/BR/RU - Add, Revise or Re-Use
 ...
 S OLDS=+LEXCHK,OLDD=$P(LEXCHK,"^",3) D NEWIEN^ZZLEXXBN(+($G(LEXSRC)),$G(CODE),.LEX) S FREQ=$$FREQ^ZZLEXXBN(LEXSRC)
 S MC=+($G(LEX("MC"))),FQ=+($G(LEX("MC"))),EX=+($G(LEX("EX"))),SO=+($G(LEX("SO"))),SM=+($G(LEX("SM")))
 S X=$$EXM^ZZLEXXBN($G(LEXEXP),.EXM,LEXSRC) I +X>0 D ADJI
 I $$TX(LEXEXP)=$$TX(OLDE),ACT="RU" S ACT="RA" D RA^ZZLEXXBE(CODE,LSTAD,LEXSRC) Q
 S TTL=$S($G(ACT)="AD":"Addition",$G(ACT)="RU":"Re-used",$G(ACT)="RA":"Re-Activation",$G(ACT)="IA":"Inactivation",1:"Revision")
 I $D(COMMIT) D
 . I $G(ACT)="FR"!($G(ACT)="BR")!($G(ACT)="RU") D IA^ZZLEXXBE(CODE,LSTAD,LEXSRC)
 ...
 . ;     Scope                     757.01       3
 . ;     Form                      757.01       4
 . I $G(CODE)="P03.4" ZW LEXEXP
 . I $G(CODE)="T82.817D" ZW LEXEXP
 . I '$D(^LEX(757.01,EX,0)) D
 . . N DA,DIERR,FDA,FDAI,IENS,COM K DIERR,FDA,DA S DA=+($G(EX)),IENS=("+1,")
...
WRT ;   Write Findings
 ...
 N ACT S ACT=AC,EACT=$$EACT^ZZLEXXBA($G(ACT)),EACT=$$TM(("Lexicon "_EACT),"s") S:EACT["Revision" EACT="Lexicon Expression Revision"
 W !!," ",$G(CODE),?21,$G(ACT),?26,EACT,?56,$$FMTE^XLFDT($G(LSTAD))
 ;   Write Inactive
 I $G(ACT)="IA",$L($G(LEXEXP)),$G(LSTAD)?7N D  Q
 . W !,"   Lexicon Expression:"  D
 . . K TA N I S TA(1)=$G(LEXEXP) D PR^ZZLEXXBM(.TA,(78-17))
 ...
 . . . K TA N I S TA(1)=$G(OLDE) D PR^ZZLEXXBM(.TA,(78-17))
...
ENV(X) ;   Environment
 ...
 Q 1
INA ;   Find Inactive Procedure Codes
 N ORD,IEN,TD S TD=$$DT^XLFDT,ORD=""
 F  S ORD=$O(^ICD0("ABA",31,ORD)) Q:'$L(ORD)  D
 . N IEN S IEN=0 F  S IEN=$O(^ICD0("ABA",31,ORD,IEN))  Q:+IEN'>0  D
 . . N CODE,SIEN,SEFF,STA
 . . S CODE=$P($G(^ICD0(+IEN,0)),"^",1)
 . . S STA=$$STATCHK^ICDEX(CODE,TD,31) Q:+STA>0
 . . Q:$P(STA,"^",2)<0
 . . Q:'$L($P(STA,"^",3))
