ZZLEXXBZ ;SLC/KER - Import - ICD-10-PCS - MailMan ;10/26/2017
 ;;1.0;Generic Utility;**1**;Sep 23, 1996
 ;               
 ; Global Variables
 ;    ^ZZLEXX("ZZLEXXBZ"  SACC 1.3
 ;               
 ; External References
 ;    $$FIND1^DIC         ICR   2051
 ;    $$GET1^DIQ          ICR   2056
...
SEND ; Send Message
 ...
 S TX=" Load ICD-10-PCS Procedures Terminology" S:$L($G(HFNAME)) TX=TX_" from "_$G(HFNAME) D BL,TL(TX)
...
END ;   Timing
 ...
 Q
MAIL ;   Mail
 K XMZ N DIFROM,I,USER,TX,CT,OK,SP S U="^" S XMSUB="Load ICD-10-PCS Procedure Terminology"
 S USER=$$NAM(+($G(DUZ))) G:'$L(USER) MAILQ S XMY(USER)="" S XMY("G.LEXICON@DNS    .DOMAIN")=""
 S I=0 F  S I=$O(^ZZLEXX("ZZLEXXBZ",$J,"MSG",I)) Q:+I'>0  D
 . S ^ZZLEXX("ZZLEXXBZ",$J,"MSG",I)=$E($G(^ZZLEXX("ZZLEXXBZ",$J,"MSG",I)),1,79)
 S XMTEXT="^ZZLEXX(""ZZLEXXBZ"","_$J_",""MSG""," N MSG
 S XMDUZ=.5 D ^XMD
MAILQ ;   Mail Quit
 S:$D(ZTQUEUED) ZTREQ="@" K ^ZZLEXX("ZZLEXXBZ",$J,"MSG"),%,%Z,XCNP,XMSCR,XMDUZ,XMY,XMZ,XMSUB,XMY,XMTEXT,XMDUZ
 Q
 ; 
 ; Miscellaneous
NAM(X) ;   Name
...
BL ;   Blank Line
 ...
 Q
TL(X) ;   Text Line
 N C S C=$O(^ZZLEXX("ZZLEXXBZ",$J,"MSG"," "),-1)+1
 S ^ZZLEXX("ZZLEXXBZ",$J,"MSG",C)=$G(X)
 Q
AL(X,P) ;   Text Line
 N TX,PSN,OLD,NEW,C S NEW=$G(X),PSN=+($G(P)),C=$O(^ZZLEXX("ZZLEXXBZ",$J,"MSG"," "),-1)
 S OLD=$G(^ZZLEXX("ZZLEXXBZ",$J,"MSG",C)),TX=OLD_$J(" ",(PSN-$L(OLD)))_NEW
 S ^ZZLEXX("ZZLEXXBZ",$J,"MSG",C)=TX
 Q
