ZZLEXXDM ;SLC/KER - Import - ICD-10-CM Cat - Misc ;10/26/2017
 ...
 ;               
 Q
CMOR(X) ; ICD-10-CM Diagnosis Order File
 K ^ZZLEXX("ZZLEXXDM",$J) N PATH,NOT,HAS,HFV S HFV=$G(^ZZLEXX("ZZLEXXD","CAT","DT","VER"))
 S HAS="CM;OR",NOT="ADDENDA" S:$D(CUR)&(HFV?4N) HAS=HAS_";"_HFV N CUR
 S NOT="ADDENDA",PATH=$$PATH D DIR(PATH,NOT,HAS) S X=$$SEL K ^ZZLEXX("ZZLEXXDM",$J) W !
 Q X
 ;
DIR(X,N,H) ; Get Directory Listing
 N ASIZE,BYTE,CNT,CON,CRE,CT,EDT,FDTS,FI,FILESPEC,FN,FULL
 N HAS,HDR,I,ID,NOT,OK,PATH,SCREEN,SEG,SIZE,TMP,TXT,UFN,Y
 K ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR"),^ZZLEXX("ZZLEXXDM",$J,"DEVICE","TEMP")
 S PATH=$G(X) Q:'$L(PATH)  S NOT=$G(N),HAS=$G(H) S FILESPEC=""
 S SCREEN("*.TXT")="",SCREEN("*.txt")=""
 S Y=$$LIST^%ZISH(PATH,"SCREEN","^ZZLEXX(""ZZLEXXDM"",$J,""DEVICE"",""TEMP"")")
 S HDR="File Name"_$J(" ",29)_"Effective Date"_$J(" ",6)_" Bytes"
 S HDR="File Name"_$J(" ",34)_"Date"_$J(" ",11)_" Bytes"
 S CT=0,FN="" F  S FN=$O(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","TEMP",FN)) Q:'$L(FN)  D
 . N ASIZE,BYTE,CRE,CNT,CON,EDT,EFFD,FDTS,FI,I,OK,SEG,SIZE,TMP,TXT,UFN,FILFY,LEXFY
 . S OK=1 I $L($G(NOT)) D
 ...
 . S LEXFY=$P($$TI(FI),"^",1)
 . Q:+FILFY<+LEXFY
 . Q:$D(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR","B",FI))
 . S TMP=$P(TMP," ",1)
 . S CT=CT+1,TXT=FI
 ...
 . S TMP=CRE
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT)=TXT
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"NAME")=FI
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"SIZE")=SIZE
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"ASIZ")=ASIZE
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"BYTE")=BYTE
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"DATE")=CRE
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"EDAT")=EDT
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",CT,"EFFD")=EFFD
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",0)=CT
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR","B",FI,CT)=""
 . S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR","C",FN,CT)=""
 S ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR","H")=HDR
 K ^ZZLEXX("ZZLEXXDM",$J,"DEVICE","TEMP")
 Q
 ;
 ; Select from Directory Listing
SEL(X) ;   Select Hostfile
 N SEL,FND,Y S FND=+($G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",0))) Q:+FND'>0 "-1^No hostfiles found/available"
 S:+FND=1 SEL=$$ONE S:+FND>1 SEL=$$MUL S X=$S(SEL>0:("1^"_$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",+SEL,"NAME"))),1:"-1^Hostfile selection not made")
 Q X
ONE(X) ;   One Entry Found
 N Y,DIR,DIRUT,DTOUT,DUOUT,DIROUT,MAX,Y S MAX=+($G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",0))) Q:MAX'=1 "-1^No selection available"
 S DIR("A",1)=("  "_$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR","H"))),DIR("A",2)=("  "_$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",1)))
 S DIR("A",3)=" ",DIR("A")="     Ok?  (Y/N)  ",DIR("B")="Yes",DIR(0)="YAO",(DIR("?"),DIR("??"))="^D ONEH^ZZLEXXDM"
...
MUL(X) ;   Multiple Entries Found
 ...
 S MAX=$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",0)) Q:MAX'>1 "-1^No selection available"
 S HDR="        "_$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR","H")) W !!," ",MAX," hostfiles found",!
 S SEL="",(LAST,EXIT,IEN)=0
 F  S IEN=$O(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",IEN)) Q:+IEN'>0!(+EXIT>0)  D  Q:+EXIT>0  Q:+SEL>0
 . N ENT,LEXT,LEXO S ENT=$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",IEN)),LAST=IEN
 ...
 Q X
MULS(X) ;     Multiple
 N DIR,DIROUT,DIRUT,DTOUT,DUOUT,MAX,LAST,Y S MAX=$G(^ZZLEXX("ZZLEXXDM",$J,"DEVICE","DIR",0)) Q:MAX'>1 "-1^No selection available"
...
EFF(X) ; Effective Date
 N DIR,DIRB,DIROUT,DIRUT,DTOUT,DUOUT,MIN,MAX,NOW,LAT,DIRB,IN
...
OPENW(X,Y) ;   Open Device
 ...
 S HFH="ZZLEXXD"_$G(Y) D OPEN^%ZISH(HFH,HFP,HFN,"W")
 S HFOK=$G(POP) I HFOK D CLOSE Q ("0^Error opening"_HFN)
 I IO'[HFN D CLOSE Q ("0^Error opening Hostfile "_HFN)
 Q 1
CLOSE(X) ;   Close Device
 N HFH S HFH="ZZLEXXD"_$G(X) D CLOSE^%ZISH(HFH)
 Q
PIT(X)  ;   Path - Input Transform
 N TPATH,TFILE,TFSPEC,OUT,FILE,PATH,IOP S TPATH=$G(X),TFILE="ZZLEXXDM"_+($G(DUZ))_".DAT",TFSPEC=TPATH_TFILE
...
LOC(X) ;   Local Namespace
 X "S X=$ZNSPACE" Q X
HFD(X) ;   Hostfile Date
 N ORG,OUT,PSN,VAL,STR,CT,RES S ORG=$G(X) Q:'$D(X) ""  S CT=0,VAL="" F PSN=1:1:$L(ORG)  D
 . S:$E(ORG,PSN)?1N VAL=VAL_$E(ORG,PSN) Q:$E(ORG,PSN)?1N  I $E(ORG,PSN)'?1N I $L(VAL) S CT=CT+1,STR(CT)=VAL,VAL=""
...
ED(X) ;   External Date
 ...
 Q X
SHOD ;   Show Directory
 N NN,NC S NN="^ZZLEXX("""_"ZZLEXXDM"_""","_$J_",""DIR"")",NC="^ZZLEXX("""_"ZZLEXXDM"_""","_$J_",""DIR"","
 F  S NN=$Q(@NN) Q:'$L(NN)!(NN'[NC)  W !,NN,"=",@NN
 Q
PS(X) ;   Page/Continuous
 K LNCT N DIR,DIRB,DIROUT,DIRUT,DTOUT,DUOUT,MIN,MAX,NOW,LAT,DIRB,IN,TX
 S DIRB=$$RET("PS^ZZLEXXDM",DUZ,"Page/Continuous") S:$L(DIRB) DIR("B")=DIRB
 S DIR(0)="SAO^P:Page;C:Continuous"
 S DIR("A")=" Display output by Page or Continuous (P/C):  "
 S (DIR("?"),DIR("??"))="^D PSH^ZZLEXXDM"
 D ^DIR S:$D(DTOUT)!($D(DUOUT))!($D(DIROUT))!(Y'?4N) X="^"
 S X=$E($$UP^XLFSTR(Y),1) S:X="P" LNCT=""
 S TX="" S:Y="P" TX="Page" S:Y="C" TX="Continuous"
 D:$L(TX) SAV("PS^ZZLEXXDM",DUZ,"Page/Continuous",TX)
 Q X
PSH(X) ;   Page/Continuous Help
 W !,"    Enter 'P' to display the output one page at a time"
 W !,"       or 'C' to display the output continuously, consider"
 W !,"              turning 'logging' on to captrue output."
 Q
...
DIS(X) ;   Display Summary or Details
 N DIR,DIROUT,DIRUT,DUOUT,DTOUT,Y,ARY,MIN,MAX,CT,I Q:'$D(^ZZLEXX("ZZLEXXD")) "^"
 S CT=0 I $D(^ZZLEXX("ZZLEXXD","CAT","CHG")) D
 . S CT=CT+1 S ARY(CT)="SUM^ZZLEXXDF",ARY(CT,1)="Summary of Changes"
 . S CT=CT+1 S ARY(CT)="CHG^ZZLEXXDF",ARY(CT,1)="Detail Listing of Changes"
 I $D(^ZZLEXX("ZZLEXXD","CAT","CHG","WARN")) D
 . S CT=CT+1 S ARY(CT)="WARN^ZZLEXXDG",ARY(CT,1)="Warnings"
 I $D(^ZZLEXX("ZZLEXXD","CAT","LOAD")) D
 . S CT=CT+1 S ARY(CT)="LOAD^ZZLEXXDF",ARY(CT,1)="Changes Loaded"
 I $D(^ZZLEXX("ZZLEXXD","CAT","DT")) D
 . S CT=CT+1 S ARY(CT)="UPD^ZZLEXXDF",ARY(CT,1)="Progress of Read/Load"
...
TI(X) ;   Terminology Fiscal Year and Date
 N FY,IN,HF,SAB S HF=$$UP^XLFSTR($G(X)) S (X,SAB)="" Q:HF'["CM" X  S SAB="10D"
