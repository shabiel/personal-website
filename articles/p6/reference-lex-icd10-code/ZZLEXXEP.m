ZZLEXXEP ;SLC/KER - Import - ICD-10-PCS Frag - Prompts ;10/26/2017
 ...
 Q
WAR(X) ; Warnings
 N HF,GBLD,ED,CURR,TXT,LAST S HF=$$UP^XLFSTR($G(X)),GBLD=$$GBLD(HF),ED=$P($$DEFF^ZZLEXXEP(HF),"^",1),CURR=$P(GBLD,"^",1),LAST=$P(GBLD,"^",2)
 Q:ED'?7N 0  Q:LAST?7N&(+ED>LAST) 1  Q:CURR'?7N 1
 S:ED?7N&(LAST?7N)&(CURR?7N)&(ED'<LAST) TXT=$$WARLP(HF,ED,GBLD)
 S:ED?7N&(LAST?7N)&(CURR?7N)&(ED<LAST) TXT=$$WARLE(HF,ED,GBLD)
 I $D(DIROUT)!($D(DIRUT))!($D(DTOUT))!($D(DUOUT))!($G(TXT)["^") Q "^"
 S X=0 S:+TXT>0 X=1
 Q X
WARLP(X,Y,Z) ;   Warning - Loaded Previously
 ...
 S (NOM,TYP)="" S:HF["PCS" NOM="ICD-10-PCS"
 S:HF["PCS"&(HF["TAB") TYP="Code Fragments"
 S:HF["PCS"&(HF["DEFINIT") TYP="Definitions" S DIR(0)="YAO"
 S TTL=NOM S:$L($G(TYP)) TTL=TTL_" "_TYP
 S DIR("A")=" Do you want to continue?  (Y/N)  ",DIR("B")="No"
 S DIR("PRE")="S:X[""?"" X=""??""",(DIR("?"),DIR("??"))="^D WARLPH^ZZLEXXEP"
 D WARLPI,^DIR S X=Y
 Q X
...
WARLE(X,Y,Z) ;   Warning - Load Error
 ...
 S HF=$$UP^XLFSTR($G(X)),ED=$G(Y),LAST=$G(Z),CURR=$P(LAST,"^",1),LAST=$P(LAST,"^",2)
 S (NOM,TYP)=""
 S:HF["PCS" NOM="ICD-10-PCS"
 S:HF["PCS"&(HF["TAB") TYP="Code Fragments"
 S:HF["PCS"&(HF["DEFINIT") TYP="Definitions" S DIR(0)="YAO"
 S TTL=NOM S:$L($G(TYP)) TTL=TTL_" "_TYP
 S DIR("A")=" Do you want to continue?  (Y/N)  ",DIR("B")="No"
 S DIR("PRE")="S:X[""?"" X=""??""",(DIR("?"),DIR("??"))="^D WARLEH^ZZLEXXEP"
 D WARLEI,^DIR S X=Y
 Q X
...
STOP(X) ; Stop, do not continue
 N CD,CONT,CY,EXIT,FD,FMT,FY,HF,I,LATER,NOM,STR1,STR2,TD,TX1,TX2,TY,TYP,UHF,Y,YR,YRS S (LATER,HF)=$G(X),UHF=$$UP^XLFSTR(HF)
 S FY=$$FY(UHF),FD=$P(FY,"^",2),FY=$P(FY,"^",1)
 S TD=$$TI(UHF),TY=$P(TD,"^",1),TD=$P(TD,"^",2) Q:FY=TY 0
 S CD=$$FI(UHF),CY=$P(CD,"^",1),CD=$P(CD,"^",2)
 S (NOM,TYP,FMT,STR1,STR2)="" S:UHF["PCS" NOM="ICD-10-PCS"
 S:UHF["PCS" TYP="Procedure" S:UHF["PCS"&(UHF["TAB") FMT="Code Fragments"
 S:UHF["PCS"&(UHF["DEFINIT") FMT="Definitions"
 S:$L(NOM) STR1=NOM S:$L($G(TYP)) STR1=STR1_" "_$G(TYP) S:$L($G(FMT)) STR1=STR1_" "_$G(FMT) S STR1=$$TM($G(STR1))
 S:UHF["PCS" TYP="Procedural" S FMT="Terminology"
...
TI(X) ;   Terminology Fiscal Year and Date
 N FY,IN,HF,SAB S HF=$$UP^XLFSTR($G(X)) S (X,SAB)="" S SAB="10P"
 ...
 Q X
GBLD(X) ;   Global Dates
 N HF,HFD,SRC,CURR,LAST,ORD,EFF S HF=$G(X),HFD=$P($$DEFF(HF),"^",1) S SRC=31
