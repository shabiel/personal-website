ZZLEXXAP ;SLC/KER - Import - ICD-10-CM - Prompts ;10/26/2017
...
EFF(X) ; Effective Date
 ...
 S (DIR("?"),DIR("??"))="^D EFFH^ZZLEXXAP",DIR("PRE")="S:X[""?"" X=""??"""
...
TL(X) ; Test Load
 ...
 S (DIR("?"),DIR("??"))="^D TLH^ZZLEXXAP",DIR("PRE")="S:X[""?"" X=""??"""
...
CM(X) ; Commit
 ...
 S (DIR("?"),DIR("??"))="^D CMH^ZZLEXXAP",DIR("PRE")="S:X[""?"" X=""??"""
...
CMW ;   Commit Warning
 ...
 W !,?3,$G(NORM),"          ^ZZLEXX(""ZZLEXXA*"" global and start the ICD-10 load over",$G(NORM)
 W !,?3,$G(NORM),"          again.",$G(NORM)
 D KATTR
 Q
 ;
BD(X) ; Select Brief/Detailed Display
 N DIR,DIROUT,DIRUT,DTOUT,DUOUT,DIRB,ENT,COM S ENT="BD^ZZLEXXAP",COM="Brief/Detailed Display"
...
BDH ;   Select Brief/Detailed Display Help
 ...
 ;
CL(X) ; Code Listing
 N DIR,DIROUT,DIRUT,DTOUT,DUOUT,DIRB,ENT,COM,OX S ENT="LC^ZZLEXXAP",COM="Code Listing"
 S DIRB=$$RET("CL") S DIR("B")="Yes" S:$L(DIRB) DIR("B")=DIRB S DIR(0)="YAO"
 S DIR("A")=" Include code listing: (Y/N):  "
 S (DIR("?"),DIR("??"))="^D CLH^ZZLEXXAP"
...
WARLP(X,Y,Z) ;   Warning - Loaded Previously
 ...
 S DIR("PRE")="S:X[""?"" X=""??""",(DIR("?"),DIR("??"))="^D WARLPH^ZZLEXXAP"
 D WARLPI,^DIR S X=Y
 Q X
...
WARLE(X,Y,Z) ;   Warning - Load Error
 ...
 S DIR(0)="YAO",DIR("A")=" Do you want to continue?  (Y/N)  ",DIR("B")="No"
 S DIR("PRE")="S:X[""?"" X=""??""",(DIR("?"),DIR("??"))="^D WARLEH^ZZLEXXAP"
 D WARLEI,^DIR S X=Y
 Q X
