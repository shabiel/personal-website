ZZLEXXAM ;SLC/KER - Import - ICD-10-CM - Host File ;2019-01-14  12:02 PM
 ;;1.0;Generic Utility;**1**;Sep 23, 1996
 ;               
 ; Global Variables
 ;    ^LEX(757.02         SACC 1.3
 ;    ^UTILITY($J         ICR  10011
 ;    ^ZZLEXX("ZZLEXXAM"  SACC 1.3
 ;               
 ; External References
 ;    ^%ZIS               ICR  10086
 ;    HOME^%ZIS           ICR  10086
...
DIR(X,N,H,S) ; Host File Directory
 ...
 K ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR"),^ZZLEXX("ZZLEXXAM",$J,"DEVICE","TEMP")
 S PATH=$G(X) Q:'$L(PATH)  S NOT=$$UP^XLFSTR($G(N)),HAS=$$UP^XLFSTR($G(H)) S FILESPEC=""
 S SCREEN("*.TXT")="",SCREEN("*.txt")=""
 S Y=$$LIST^%ZISH(PATH,"SCREEN","^ZZLEXX(""ZZLEXXAM"",$J,""DEVICE"",""TEMP"")")
 S HDR="File Name"_$J(" ",29)_"Effective Date"_$J(" ",6)_" Bytes"
 S HDR="File Name"_$J(" ",34)_"Date"_$J(" ",11)_" Bytes"
 S CT=0,FN="" F  S FN=$O(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","TEMP",FN)) Q:'$L(FN)  D
 . N ASIZE,BYTE,CRE,CNT,CON,EDT,EFFD,FDTS,FI,I,OK,SEG,SIZE,TMP,TXT,UFN
 . S OK=1 I $L($G(NOT)) D
 ...
 . S EFFD=$$DEFF^ZZLEXXAP(FI)
 . Q:$D(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR","B",FI))
 . S TMP=$P(TMP," ",1)
 . S CT=CT+1,TXT=FI
 ...
 . S TMP=CRE
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT)=TXT
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"NAME")=FI
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"SIZE")=SIZE
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"ASIZ")=ASIZE
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"BYTE")=BYTE
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"DATE")=CRE
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"EDAT")=EDT
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",CT,"EFFD")=EFFD
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",0)=CT
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR","B",FI,CT)=""
 . S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR","C",FN,CT)=""
 S ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR","H")=HDR
 K ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","TEMP")
 Q
DIRQ ;   Directory Quit
 K ^ZZLEXX("ZZLEXXAM",$J,"DEVICE","TEMP")
 Q
...
SEL(X) ; Host File Selection
 N SEL,FND,Y S FND=+($G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",0))) I +FND'>0 K ^ZZLEXX("ZZLEXXAM",$J) Q "-1^No host files found/available"
 S:+FND=1 SEL=$$ONE S:+FND>1 SEL=$$MUL S X=$S(SEL>0:("1^"_$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",+SEL,"NAME"))),1:"-1^Host file selection not made")
 K ^ZZLEXX("ZZLEXXAM",$J) Q X
ONE(X) ;   One Host File Found
 N Y,DIR,DIRUT,DTOUT,DUOUT,DIROUT,MAX S MAX=+($G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",0))) Q:MAX'=1 "-1^No selection available"
 S DIR("A",1)=("  "_$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR","H"))),DIR("A",2)=("  "_$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",1)))
 S DIR("A",3)=" ",DIR("A")="     Ok?  (Y/N)  ",DIR("B")="Yes",DIR(0)="YAO",(DIR("?"),DIR("??"))="^D ONEH^ZZLEXXAM"
...
MUL(X) ;   Multiple Host Files Found
 ...
 S MAX=$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",0)) Q:MAX'>1 "-1^No selection available"
 S HDR="        "_$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR","H")) W !!," ",MAX," host files found",!
 S SEL="",(LAST,EXIT,IEN)=0
 F  S IEN=$O(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",IEN)) Q:+IEN'>0!(+EXIT>0)  D  Q:+EXIT>0  Q:+SEL>0
 . N ENT,LEXT,LEXO S ENT=$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",IEN)),LAST=IEN
...
MULS(X) ;   Multiple Host Files Prompt
 N DIR,DIROUT,DIRUT,DTOUT,DUOUT,MAX,LAST S MAX=$G(^ZZLEXX("ZZLEXXAM",$J,"DEVICE","DIR",0)) Q:MAX'>1 "-1^No selection available"
...
PIT(X)  ;   Path - Input Transform
 N TPATH,TFILE,TFSPEC,OUT,FILE,PATH,IOP S TPATH=$G(X),TFILE="ZZLEXXAM"_+($G(DUZ))_".DAT",TFSPEC=TPATH_TFILE
