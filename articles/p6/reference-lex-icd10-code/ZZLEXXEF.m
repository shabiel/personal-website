ZZLEXXEF ;SLC/KER - Import - ICD-10-PCS Frag - Device ;10/26/2017
...
PIT(X)  ;   Path - Input Transform
 N TPATH,TFILE,TFSPEC,OUT,FILE,PATH,IOP S TPATH=$G(X),TFILE="ZZLEXXEA"_+($G(DUZ))_".DAT",TFSPEC=TPATH_TFILE
...
HFN(X,Y) ;   Hostfile Name
 N SYS,TY,FY
 S SYS=$G(X),TY=$G(Y),FY=$G(^ZZLEXX("ZZLEXXE","PCS","CHG",DT)) S:FY?7N FY=+($E(FY,1,3))+1700 S:FY'?4N FY=+($E($$DT^XLFDT,1,3))+1700
